...
export default {
  ...
  methods: {
    ...
    addCredential(user,pass){
      ...
      let credentials = localStorage.getItem('cretendials');
      ...
      localStorage.setItem('cretendials', JSON.stringify(credentials));
    ...
    registrar(){
        ...
        if(erros.length > 0){
          ...
          this.addCredential(this.login.user,this.login.pass)
